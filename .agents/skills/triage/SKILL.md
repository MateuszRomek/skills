...
These are canonical role names. The actual label strings used in the issue tracker may differ. The mapping should have been provided to you. If not, tell the user to run `/setup-engineering-workspace`.
...
4. **Grill (if needed).** If the request needs fleshing out, invoke **grilling** and **domain-modeling**, then grill it into shape a round of questions at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land.
