...
2. **Source wave.** Create one semantic read unit per feature file and dispatch the units as `bulk-reader` through [`HOST-COMPATIBILITY.md`](../../agent-mode/HOST-COMPATIBILITY.md). The route's `partition` semantics map them onto the setup-owned roster within the current branch budget. Workers remain read-only: they never drive the app or edit files. Pass any descendant budget reserved for their branch. Each returns: feature summary / source entry points / likely drift or none / one concise live-verification recipe.
