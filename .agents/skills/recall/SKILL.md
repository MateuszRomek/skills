...
Keep it tight and on-topic. Read only what the in-scope tasks need, then stop. Heavy reading is dispatched as semantic units through the configured `bulk-reader` route. The main task keeps only findings and the final brief.
...
Resolve history and delegation capabilities through [`HOST-COMPATIBILITY.md`](../../agent-mode/HOST-COMPATIBILITY.md). Use the host's task or conversation APIs first. Otherwise use only a workspace-scoped transcript path supplied by the host. If neither exists, reconstruct from the visible conversation and live state, and state that limitation.
...
3. Divide chat history into coherent corpus slices and dispatch them as `bulk-reader`. Tell every resolved worker to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first, read only matching tasks and relevant regions, and skip the current task plus obvious noise. Each returns the same schema, one block per task: topic, user goal, decisions, open threads, struggles and corrections, and artifacts, each citing the task UUID. The raw transcripts stay with the workers; the coordinator keeps only findings.
4. Sweep the shared record whenever the topic names a feature, file, subsystem, area, or bug. Hand semantic source units to the **why** skill with the question reframed as "what's the current state, what's been tried and didn't hold, and what are users still reporting". Reuse its source playbooks and routing contract. Null results are findings; unavailable sources are explicit gaps. Skip this step only for pure activity recall with no named target.
