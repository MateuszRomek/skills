...
Write `.agents/skills/verify-<app>/SKILL.md` with YAML frontmatter (`name: verify-<app>` and a `description` that names the app, the surface, and when to reach for it — without frontmatter the skill never registers) and these sections, each grounded in what the interview actually found (no placeholders left). Add a relative `.claude/skills/verify-<app>` symlink when this repository maintains the existing Claude compatibility layer.
