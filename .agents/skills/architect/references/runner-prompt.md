...
The coordinator passes this file to every resolved `architect-runners` worker and fills in the variable inputs around it: the task, the Phase A grounding artifacts, the isolated working directory, and the path to write outputs. The working directory is a git worktree when available, otherwise a per-runner subdirectory under the sketch dir; what matters is independence between candidates.

You are producing an independent candidate design in architect's configured comparison. Read the **architect** skill in full first; that is the workflow you are inside. Output a candidate design package: type sketch, function signatures, module map, and prose rationale shaped per [`rationale-template.md`](rationale-template.md).
...
Produce the best independent design you can make; do not hedge against possible alternatives. Differences between returned candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
