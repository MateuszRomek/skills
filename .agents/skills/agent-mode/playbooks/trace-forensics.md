...
1. Identify the format and load it with the right tool. Dispatch large-artifact read units as `bulk-reader` through [`HOST-COMPATIBILITY.md`](../../../agent-mode/HOST-COMPATIBILITY.md). Keep only reduced findings in the main task (the **principle-guard-the-context-window** skill).
