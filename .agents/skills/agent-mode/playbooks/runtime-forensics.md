...
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. Dispatch large-artifact read units as `bulk-reader` through [`HOST-COMPATIBILITY.md`](../../../agent-mode/HOST-COMPATIBILITY.md) (the **guard-the-context-window** principle skill). Keep only reduced findings in the main task.
