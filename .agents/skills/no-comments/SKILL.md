...
1. Spawn the host's `Comment Sicko` custom agent when available. Otherwise spawn one subagent with `.agents/agent-mode/agents/comment-sicko.md` as its instruction file. Pass the scope. Do not restate its rules.
