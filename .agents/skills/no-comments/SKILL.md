...
1. Dispatch the review scope as `comment-sicko` through [`HOST-COMPATIBILITY.md`](../../agent-mode/HOST-COMPATIBILITY.md). Use `.agents/agent-mode/agents/comment-sicko.md` as every resolved worker's instruction file, or the host's equivalent named custom agent. The configured roster is complete; do not add a fallback. Pass the scope without restating its rules.
